...
public class MetalBarrelScreen extends AbstractContainerScreen<MetalBarrelContainer> {
  ...
  protected void init() {
    ...
    this.usernameInput = new EditBox(this.font, 5, this.height - 115, 80, 12, new TextComponent("Input username"));
  ...
  public void render(@NotNull PoseStack poseStack, int x, int y, float partialTicks) {
    ...
    if (BarrelLockClientPacket.isOwner) {
      ...
      if (!BarrelLockClientPacket.isLocked) {
        this.addRenderableWidget(new BarrelLockButton(30, this.height - 35, 25, 30, new TranslatableComponent("metalbarrels.screen.lock"),
                (press) -> BarrelNetwork.CHANNEL.sendToServer(new BarrelLockServerPacket(blockPos, true))));
      } else {
        this.addRenderableWidget(new BarrelUnlockButton(30, this.height - 35, 25, 30, new TranslatableComponent("metalbarrels.screen.unlock"),
                (press) -> BarrelNetwork.CHANNEL.sendToServer(new BarrelLockServerPacket(blockPos, false))));
      }

      this.addRenderableWidget(new BarrelAddPlayerButton(5, this.height - 68, 80, 30, new TextComponent("Whitelist Player"),
              (press) -> {
                renderUsernameInput = true;
                isWhitelist = true;
      }));

      this.addRenderableWidget(new BarrelRemovePlayerButton(5, this.height - 101, 80, 30, new TextComponent("Remove Player"),
              (press) -> {
                renderUsernameInput = true;
                isWhitelist = false;
      }));
